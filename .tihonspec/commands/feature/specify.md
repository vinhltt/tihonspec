...
3. **Check feature description**:
   ...
   - If description EMPTY or MISSING:
     ERROR: "Description required. Usage: /feature.tihonspec {task-id} {description}"
...
4. **Infer task_id from conversation context** (only if task_id missing):
   ...
   **If NO context found**:
   ...
   Usage: /feature.tihonspec <task-id> <description>
   Example: /feature.tihonspec pref-001 "Add user authentication"
...
5. **Validate task_id format**:
   ...
   - Examples:
     ...
     - ✅ `/feature.tihonspec hotfix/pref-123 "Fix bug"` → task_id: `hotfix/pref-123`
     - ❌ `/feature.tihonspec "Add auth"` → ERROR (no task ID)

6. **Execute Script**:
   ```bash
   bash .tihonspec/scripts/bash/create-new-feature.sh <task-id> <description>
   ```
   ...
   - Creates feature directory: `.tihonspec/{folder}/{task-id}/`
   - Copies spec template to: `.tihonspec/{folder}/{task-id}/spec.md`
...
### Step 1: Load `.tihonspec/templates/spec-template.md` to understand required sections.
...
6. Report completion with branch name, spec file path, checklist results, and readiness for the next phase (`/feature.clarify` or `/feature.plan`).
